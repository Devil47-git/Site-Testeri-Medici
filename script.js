...
const TEST_CATALOG_KEY = 'medici-test-catalog-v2';
...
async function saveRemoteGrant(callsign, grantedTests, remove = false) {
  if (!currentUser?.discordId) throw new Error('Sesiunea nu conține Discord ID.');
  const response = await fetch('/api/access/grants', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ requesterId: currentUser.discordId, callsign, grantedTests, remove }) });
  const payload = await response.json();
...
const COOLDOWN_KEY = 'medici-test-cooldowns';
function cooldownUntil(testName) { return Number(JSON.parse(localStorage.getItem(COOLDOWN_KEY) || '{}')[testName] || 0); }
function setCooldown(testName, days) { const values = JSON.parse(localStorage.getItem(COOLDOWN_KEY) || '{}'); values[testName] = Date.now() + days * 24 * 60 * 60 * 1000; localStorage.setItem(COOLDOWN_KEY, JSON.stringify(values)); }
function cooldownText(timestamp) { return timestamp > Date.now() ? `Cooldown până la ${new Date(timestamp).toLocaleDateString('ro-RO')}` : ''; }
...
function renderView(view) {
  ...
  if (view === 'tests') viewContent.innerHTML = `<div class="panel view-panel"><div class="panel-head"><div><h2>${title}</h2><p class="muted">Sunt afișate doar testele la care ai acces, pentru ${normalizeCallsign(currentUser?.callsign)}.</p></div></div><div class="test-cards">${allowedForUser(currentUser).map(test => { const until = cooldownUntil(test); return `<article class="test-card"><h3>${test}</h3><p class="muted">${testDefinitions[test]?.description || 'Test disponibil.'}</p>${until > Date.now() ? `<p class="cooldown">${cooldownText(until)}</p>` : `<button class="primary" data-test="${test}">Deschide testul</button>`}</article>`; }).join('') || '<div class="empty-state">Nu ai teste disponibile.</div>'}</div></div>`;
...
function openTest(testName) {
  ...
  viewContent.innerHTML = `<div class="panel view-panel"><div class="panel-head"><div><p class="eyebrow">TEST ÎN DESFĂȘURARE</p><h2>${testName}</h2><p class="muted">Acces confirmat pentru ${normalizeCallsign(currentUser?.callsign)}.</p></div><button class="outline" id="back-to-tests">← Înapoi</button></div><p class="muted">${definition.description}</p><p class="test-instructions">${definition.instructions || ''}</p>${images ? `<div class="test-images">${images}</div>` : ''}${cases ? `<label>Cazul ales de candidat<select id="case-select">${cases}</select></label><div id="case-steps" class="case-steps"></div>` : ''}${questions.length ? `<form id="test-form" class="question-list">${questions.map((question, index) => `<fieldset><legend>${index + 1}. ${question.text}</legend><label class="answer-check"><input type="checkbox" data-wrong="${index}"> Răspuns greșit</label><textarea required name="q-${index}" rows="3" placeholder="Notează răspunsul candidatului…"></textarea></fieldset>`).join('')}<p>Greșeli: <strong id="wrong-count">0</strong> / ${definition.maxWrong ?? '—'}</p><button class="primary" type="submit">Finalizează testul</button></form>` : '<div class="test-runner"><p>Testul este configurat, dar încă nu are întrebări introduse.</p></div>'}</div>`;
  ...
  const form = document.querySelector('#test-form'); if (form) form.onsubmit = event => { event.preventDefault(); const wrong = wrongInputs.filter(item => item.checked).length; const limit = Number(definition.maxWrong ?? Infinity); const failed = wrong > limit; if (failed && definition.cooldownDays) setCooldown(testName, definition.cooldownDays); form.innerHTML = `<p class="${failed ? 'failure-message' : 'success-message'}">${failed ? `Candidatul a picat testul. ${cooldownText(cooldownUntil(testName))}` : 'Testul a fost finalizat. Verifică manual proba practică și notează ADMIS/RESPINS.'}</p>`; };
}
renderRows();
