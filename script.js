...
const TEST_CATALOG_KEY = 'medici-test-catalog-v3';
...
function openTest(testName) {
  ...
  viewContent.innerHTML = `<div class="panel view-panel"><div class="panel-head"><div><p class="eyebrow">GHID PENTRU TESTER</p><h2>${testName}</h2><p class="muted">Acces permanent pentru testerul conectat: ${normalizeCallsign(currentUser?.callsign)}.</p></div><button class="outline" id="back-to-tests">← Înapoi</button></div><p class="muted">${definition.description}</p><p class="test-instructions">${definition.instructions || ''}</p>${images ? `<div class="test-images">${images}</div>` : ''}${cases ? `<label>Cazul ales de candidat<select id="case-select">${cases}</select></label><div id="case-steps" class="case-steps"></div>` : ''}${practical ? `<label>Probă practică<select id="practical-select">${practical}</select></label><div id="practical-steps" class="case-steps"></div>` : ''}${questions.length ? `<form id="test-form" class="question-list"><label>Identificator candidat (opțional)<input id="candidate-id" type="text" placeholder="Callsign / nume candidat"></label>${questions.map((question, index) => `<fieldset><legend>${index + 1}. ${question.text}</legend><div class="correct-answer"><b>Răspuns corect / criteriu:</b><span>${question.answer || 'Verifică ghidul.'}</span></div><label class="answer-check"><input type="checkbox" data-wrong="${index}"> Răspuns greșit</label></fieldset>`).join('')}<p>Greșeli: <strong id="wrong-count">0</strong> / ${definition.maxWrong ?? '—'}</p><button class="primary" type="submit">Finalizează evaluarea</button></form>` : '<div class="test-runner"><p>Acest ghid nu are întrebări teoretice configurate.</p></div>'}</div>`;
  ...
  const form = document.querySelector('#test-form'); if (form) form.onsubmit = event => { event.preventDefault(); const wrong = wrongInputs.filter(item => item.checked).length; const limit = Number(definition.maxWrong ?? Infinity); const failed = wrong > limit; form.innerHTML = `<p class="${failed ? 'failure-message' : 'success-message'}">${failed ? 'Candidatul a picat testul conform numărului de greșeli selectat.' : 'Evaluarea a fost înregistrată. Testerul decide ADMIS/RESPINS pentru proba completă.'}</p>`; };
}
renderRows();
